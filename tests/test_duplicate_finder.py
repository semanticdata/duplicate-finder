...
from main import calculate_file_hash, find_duplicates, process_file
...
def test_process_file(temp_dir):
    ...
    result = process_file((str(small_file), 100, 65536))
    assert result is None

    # Test with file larger than min_size
    large_file = temp_dir / "large.txt"
    create_test_file(large_file, "large" * 100)
    result = process_file((str(large_file), 10, 65536))
...
def test_find_duplicates(temp_dir):
    ...
    duplicates, _, _, _ = find_duplicates(
        str(temp_dir),
